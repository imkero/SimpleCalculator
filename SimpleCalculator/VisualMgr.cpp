...
VisualMgr::VisualMgr() :
	KeyBoardFont("Microsoft YaHei UI", 15),
	KeyBoardDigitFont("Microsoft YaHei UI", 18, QFont::DemiBold),
	PanelResultFont("Microsoft YaHei UI", 20),
...
{
	updateParamCache();
	ExprPosiiton = QPoint(0, PanelExprHeight.Ascent);
}

void VisualMgr::updateParamCache()
{
	QFontMetrics exprFontMetrics(PanelExprFont);
	QFontMetrics exprSubFontMetrics(PanelExprFont);

	PanelExprHeight = DualHeight(exprFontMetrics.ascent(), exprFontMetrics.descent());
	PanelSubExprHeight = DualHeight(exprSubFontMetrics.ascent(), exprSubFontMetrics.descent());
...
void VisualMgr::drawToken(QPainter *painter, QPoint &point, TokenType token)
{
	char c[2];
	c[0] = EnumConvert::token2char(token);
	c[1] = '\0';
	painter->drawText(point + QPoint(0, painter->fontMetrics().descent()), c);
	point.rx() += PanelTokenWidth[token];
}
